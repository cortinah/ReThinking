...
$$
\frac{1}{2}=\sum_{k=51}^{k=100}\binom{100}{k}p^{k}(1-p)^{100-k}
$$
Find the root in (0, 1) of:
$$
\sum_{k=51}^{k=100}\binom{100}{k}p^{k}(1-p)^{100-k}-\frac{1}{2}=0
$$
```{r Q2prob}
senators <- 60:100
...
We use equation (1) to calculate the probability of winning a supermajority in a given election. This approximately 4.7%. Accordingly, the Programmers can expect to win 4-5 supermajorities over a century.
```{r Q3howmanysuper}
p <- 0.5116
senators <- 60:100
...
ggplot(optimal,aes(x=Year,y=Weight)) +geom_line(color='blue') +geom_point(color='red') +theme_bw() +labs(title='Riddler Classic: How Far Would You Go To Rig A Coin Flip?', subtitle='Optimal coin weight avoiding 99% detection probability',caption='Source: @cortinah, 11/2/2018')
```
