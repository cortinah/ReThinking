...
ggplot(krem, aes(x=date, y=mean_temp)) +geom_line(alpha=0.8)  +
  ...
  scale_y_continuous('Mean Temperature Cº', limits=c(-20, 30), breaks=seq(-20, 30, 10))  +
...
```{r anom, message=F, warning=F, error=F, fig.width=9.5, fig.height=6}
...
### Freezing days
```{r freeze, message=F, warning=F, error=F, fig.width=9.5, fig.height=6}
...
### Hot days
```{r hot, message=F, warning=F, error=F, fig.width=9.5, fig.height=6}
...
```{r excess, message=F, warning=F, error=F, fig.width=9.5, fig.height=6}
...
ggplot(excess_degrees, aes(x=year, y=excess_degrees_total)) +geom_line(alpha=0.7)  +
  ...
  geom_smooth(size=1.5, color='red') +
  labs(title="Kremsmünster, Austria: 1876-2018 Annual Sum of 'Excess Degree' Days", subtitle = 'Annual sum of (daily high - 30 Cº). Loess trend in red',
       caption='Source: https://www.ecad.eu/')
```

## Forecasting with Fable

### Forecasting temperature anomaly with ETS model
