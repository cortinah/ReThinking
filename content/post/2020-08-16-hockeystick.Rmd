...
```{r carbon, out.width='70%', fig.retina=2}
library(hockeystick)
ml_co2 <- get_carbon()
plot_carbon(ml_co2)
```
...
```{r temp, out.width='70%', fig.retina=2}
anomaly <- get_temp()
plot_temp(anomaly)
```

Visualize warming using Ed Hawkins styled "warming stripes":
```{r stripes, out.width='70%', fig.retina=2}
warming_stripes()
```
```{r stripes2, out.width='63%', fig.retina=2}
...
```{r sl, out.width='70%', fig.retina=2}
gmsl <- get_sealevel()
plot_sealevel(gmsl)
```
...
```{r si, out.width='70%', fig.retina=2}
seaice <- get_seaice()
plot_seaice(seaice)
```
...
```{r paleo, out.width='75%', fig.retina=2}
vostok <- get_paleo()
plot_paleo(vostok)
```
